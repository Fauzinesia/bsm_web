...
// Handle inline status updates BEFORE any output
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_status') {
    if (session_status() === PHP_SESSION_ACTIVE) { session_write_close(); }
    $id = (int)($_POST['id'] ?? 0);
    $newStatus = $_POST['status'] ?? '';
    $allowed = ['Dijadwalkan','Proses','Selesai'];
    if ($id > 0 && in_array($newStatus, $allowed, true)) {
        try {
            $koneksi->begin_transaction();
            $stmt = $koneksi->prepare('UPDATE tb_maintenance SET status = ? WHERE id_maintenance = ?');
            if (! $stmt) { throw new Exception('Gagal menyiapkan query.'); }
            $stmt->bind_param('si', $newStatus, $id);
            $stmt->execute();
            $stmt->close();
            $koneksi->commit();
            header('Location: maintenance.php?status=success&message=' . urlencode('Status diperbarui menjadi ' . $newStatus));
            exit;
        } catch (Throwable $e) {
            if ($koneksi->errno) { $koneksi->rollback(); }
            header('Location: maintenance.php?status=error&message=' . urlencode('Gagal memperbarui status: ' . $e->getMessage()));
            exit;
        }
    } else {
        header('Location: maintenance.php?status=error&message=' . urlencode('Parameter tidak valid untuk pembaruan status.'));
        exit;
    }
}
$page_title = 'Manajemen Maintenance Kendaraan';
        ...
        <div class="page-header">
          <div class="page-block">
            <div class="page-header-title">
              <h5 class="mb-0 font-medium">Manajemen Maintenance Kendaraan</h5>
            </div>
            <ul class="breadcrumb">
          ...
          <div class="col-span-12 md:col-span-6 xl:col-span-4">
            <div class="card">
              <div class="card-body">
                ...
                <p class="text-sm text-muted mt-3">Riwayat perawatan kendaraan di PT BSM.</p>
              </div>
            </div>
          </div>
          <div class="col-span-12 md:col-span-6 xl:col-span-4">
              ...
              <div class="card-header flex justify-between items-center">
                ...
                <div class="flex gap-2">
                  ...
                  <a href="tambah.php" class="btn btn-primary">
                    <i class="ti ti-plus me-2"></i>Tambah Maintenance
                  </a>
                </div>
              </div>
              <div class="card-body">
                  ...
                  <table class="table table-hover">
                    ...
                    <tbody>
                      <?php if (count($maintenance) === 0): ?>
                        <tr>
                          <td colspan="6" class="text-center text-muted">Belum ada data maintenance.</td>
                        </tr>
                      <?php else: ?>
                        <?php foreach ($maintenance as $item): ?>
                          <tr>
                            ...
                            <td class="text-end">
                              <div class="btn-group">
                                <a href="edit.php?id=<?php echo (int)$item['id_maintenance']; ?>" class="btn btn-sm btn-outline-primary"><i class="ti ti-edit"></i> Edit</a>
                                <a href="hapus.php?id=<?php echo (int)$item['id_maintenance']; ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Yakin hapus data ini?');"><i class="ti ti-trash"></i> Hapus</a>
                                <?php $cur = $item['status'] ?? 'Dijadwalkan'; ?>
                                <?php if ($cur !== 'Selesai'): ?>
                                  <a href="../operasional/tambah.php?id_kendaraan=<?php echo (int)$item['id_kendaraan']; ?>&ref_maintenance=<?php echo (int)$item['id_maintenance']; ?>"
                                     class="btn btn-sm btn-outline-info">
                                    <i class="ti ti-cash"></i> Biaya Operasional
                                  </a>
                                <?php endif; ?>
                                <?php if ($cur === 'Dijadwalkan'): ?>
                                  <form method="POST" class="d-inline">
                                    <input type="hidden" name="action" value="update_status" />
                                    <input type="hidden" name="id" value="<?php echo (int)$item['id_maintenance']; ?>" />
                                    <input type="hidden" name="status" value="Proses" />
                                    <button type="submit" class="btn btn-sm btn-outline-warning"><i class="ti ti-settings"></i> Proses</button>
                                  </form>
                                <?php elseif ($cur === 'Proses'): ?>
                                  <form method="POST" class="d-inline">
                                    <input type="hidden" name="action" value="update_status" />
                                    <input type="hidden" name="id" value="<?php echo (int)$item['id_maintenance']; ?>" />
                                    <input type="hidden" name="status" value="Dijadwalkan" />
                                    <button type="submit" class="btn btn-sm btn-outline-secondary"><i class="ti ti-calendar"></i> Jadwalkan</button>
                                  </form>
                                  <form method="POST" class="d-inline">
                                    <input type="hidden" name="action" value="update_status" />
                                    <input type="hidden" name="id" value="<?php echo (int)$item['id_maintenance']; ?>" />
                                    <input type="hidden" name="status" value="Selesai" />
                                    <button type="submit" class="btn btn-sm btn-outline-success"><i class="ti ti-check"></i> Selesai</button>
                                  </form>
                                <?php endif; ?>
                              </div>
                            </td>
