...
$page_title = 'Inspeksi Kendaraan (User)';
...
if ($filterSearch !== '') {
    $sqlList .= ' AND (k.nomor_polisi LIKE ? OR k.nama_kendaraan LIKE ?)';
...
if (!empty($params)) {
    ...
    if ($stmt) {
        ...
        while ($row = $result->fetch_assoc()) { $inspeksi[] = $row; }
        $stmt->close();
    }
} else {
        ...
        <div class="page-header">
          <div class="page-block">
            <div class="page-header-title">
              <h5 class="mb-0 font-medium">Inspeksi Kendaraan</h5>
            </div>
            <ul class="breadcrumb">
                ...
                <form method="GET" class="mb-4">
                  <div class="grid grid-cols-12 gap-3">
                    <div class="col-span-12 md:col-span-5">
                      <input type="text" name="search" class="form-control" placeholder="Cari nomor polisi/nama..." value="<?php echo e($filterSearch); ?>">
                    </div>
                        ...
                        <td class="text-end">
                          <div class="btn-group">
                                <?php
                                  ...
                                  if ($stmtRef = $koneksi->prepare('SELECT COUNT(*) AS cnt FROM tb_maintenance WHERE deskripsi LIKE ?')) {
                                    $like = '%Ref Inspeksi #' . (int)$item['id_inspeksi'] . '%';
                                    $stmtRef->bind_param('s', $like);
                                    $stmtRef->execute();
                                    $refRes = $stmtRef->get_result();
                                    if ($refRes && ($refRow = $refRes->fetch_assoc())) {
                                      $hasMaintenance = ((int)$refRow['cnt'] > 0);
                                    }
                                    if ($refRes) { $refRes->free(); }
                                    $stmtRef->close();
                                  }
                                ?>
                                <?php if (! $hasMaintenance): ?>
                                  <a href="../maintenance/tambah.php?id_inspeksi=<?php echo (int)$item['id_inspeksi']; ?>&id_kendaraan=<?php echo (int)$item['id_kendaraan']; ?>" class="btn btn-sm btn-outline-success"><i class="ti ti-tools"></i> Ajukan Maintenance</a>
                                <?php endif; ?>
                          </div>
                        </td>
                          </tr>
